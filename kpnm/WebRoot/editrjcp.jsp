			...
			<div class="main" align="center">
				...
				<div class="sysms" >
					...
					<div id="chinese" style="width: 100%">
							...
							<div class="titlepser">
								<div  class="t">
									...
									<input id="namech" type="text" name="namech" value="<s:property value='spe.name'/>" placeholder="请输入软件产品名称"/><br>
									<span style="margin-left: 3px">Product Name：</span>
									<input id="nameen" type="text" name="nameen" value="<s:property value='spe.nameen'/>" placeholder="input the software product name here"/>
								</div>
							</div>
							 <section id="editor">
							      <div id='edit' style="margin-top: 2px;">
							      	<img alt="" src="<s:property value='spe.photo'/>">
							      </div>
							  </section>
					</div>
					...
					<div style="clear: both;height: 40px;background: rgba(137, 238, 223, 0.06);margin-top: 10px">
						<div id="<s:property value='id'/>" style="height: 40px;line-height:30px;text-align:right">
							<span id="savecn" style="padding:6px;">保存修改</span>
						</div>
					</div>
				</div>
			</div>
				  ...
				  var contentch = $('#edit').children("div").eq(1).html();
					...
					if(namech)
					{
					 	 $.ajax({  
						            url: 'saverjcp',  
						            type: 'post',  
						            data: {savecpid:id,savenamech:namech,savenameen:nameen,savephoto:imglink},  
						            success: function(data)
						            {
					            		if(data.saverjcpresult == 'ok')
						            	{
						            		 alert("保存成功");
						            	}
						            }
